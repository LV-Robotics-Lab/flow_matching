...
from lv_flow_matching.tools.tactile_feat import TACTILE_FEATURE_DIM, extract_tactile_deformation
...
def require_complete_zarr_source(zarr_root: Any, *, zarr_path: str) -> None:
    """Reject incomplete new sources while preserving marker-free legacy data."""
    source_format = zarr_root.attrs.get("format")
    has_completion_marker = "complete" in zarr_root.attrs
    complete = zarr_root.attrs.get("complete")
    has_invalid_marker = has_completion_marker and complete is not True
    if has_invalid_marker or (
        source_format == "prometheus_flow_matching_v1" and complete is not True
    ):
        raise ValueError(
            f"Flow Matching source is incomplete at {zarr_path}; "
            "finish or regenerate preprocessing before training"
        )
...
class ZarrDataset(Dataset):
    def __init__(
        ...
        require_complete_zarr_source(self.zarr_root, zarr_path=self.zarr_path)
