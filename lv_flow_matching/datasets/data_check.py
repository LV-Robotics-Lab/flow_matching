...
_FLOW_MATCHING_ROOT = str(Path(__file__).resolve().parents[2])
_DATASETS_DIR = os.path.dirname(os.path.abspath(__file__))
from lv_flow_matching.datasets.zarr_dataset import ZarrDataset
from lv_flow_matching.tools.tactile_feat import TACTILE_BUNDLE_ORDER
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default=os.path.join(_FLOW_MATCHING_ROOT, "configs", "train", "config.yaml"),
    )
