...
from lv_flow_matching.infer.preprocess import build_policy_memory_input
...
def main() -> None:
    ...
    final_global_cond = torch.cat([obs_cond, policy_global], dim=-1)
    assert final_global_cond.shape == (b, policy.global_cond_dim)
    assert policy.global_cond_dim == 2 * policy.cond_dim
