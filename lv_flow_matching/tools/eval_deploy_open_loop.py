...
from lv_flow_matching.infer import FMInferenceRuntime, load_run_config, obs_from_zarr_window
from lv_flow_matching.utils.train_utils import cfg_get, set_seed


def build_eval_dataset(cfg: dict) -> ZarrDataset:
    data_cfg = dict(cfg["data"])
    data_cfg["fit_normalizer"] = False
    # Deployment inference encodes current RGB online. Evaluation must expose
    # images even when the training config points at a precomputed cache.
