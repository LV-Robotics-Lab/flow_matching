...
if [[ "$only" == "all" || "$only" == "dino" ]]; then
    run_test dino lv_flow_matching.tools.bench_dino_latency
fi
if [[ "$only" == "all" || "$only" == "memory" ]]; then
    run_test memory lv_flow_matching.tools.bench_memory_latency
fi
if [[ "$only" == "all" || "$only" == "policy" ]]; then
    run_test policy lv_flow_matching.tools.bench_policy_latency
fi
if [[ "$only" == "all" || "$only" == "full" ]]; then
    run_test full_pipeline lv_flow_matching.tools.bench_full_pipeline_latency
fi
if [[ "$only" == "all" || "$only" == "concurrent" ]]; then
    run_test concurrent_realistic lv_flow_matching.tools.bench_concurrent_latency --scenario realistic
    if [[ $skip_stress -eq 0 ]]; then
        run_test concurrent_stress lv_flow_matching.tools.bench_concurrent_latency --scenario stress
    fi
fi
