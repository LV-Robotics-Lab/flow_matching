...
```bash
python -m lv_flow_matching.tools.bench_policy_latency \
    --architecture-only \
    --num-views 3 \
    --device cuda:0
```
...
```bash
python -m lv_flow_matching.tools.bench_dino_latency \
    --run-dir /path/to/run_dir \
    --device cuda:0
```
...
```bash
python -m lv_flow_matching.tools.bench_memory_latency \
    --run-dir /path/to/run_dir \
    --device cuda:0
```
...
```bash
python -m lv_flow_matching.tools.bench_full_pipeline_latency \
    --run-dir /path/to/run_dir \
    --device cuda:0 \
    --camera-period-ms 33 \
...
```bash
python -m lv_flow_matching.tools.bench_concurrent_latency \
    --run-dir /path/to/run_dir \
    --device cuda:0 \
    --scenario realistic \
    --duration-seconds 30 \
