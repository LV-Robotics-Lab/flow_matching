...
from lv_flow_matching.infer.preprocess import cuda_memory_snapshot, tensor_mib
from lv_flow_matching.infer.runtime import FMInferenceRuntime
from lv_flow_matching.models.fm.encoders.dino_v2 import DinoV2SmallEncoder
...
def main() -> None:
    if len(sys.argv) > 2:
        raise SystemExit(
            "Usage: python -m lv_flow_matching.tools.test_dino_global_local_memory [RUN_DIR]"
        )
    ...
    with torch.inference_mode():
        ...
        # extract_backbone_feat now returns full tokens (B,257,D); CLS is token 0.
        current_global = model.extract_backbone_feat(inspection_image)[:, :1]
    ...
    global_buffer.stop()
    torch.cuda.synchronize()
    global_snapshot = cuda_memory_snapshot("global_buffer_128", device)
    ...
    describe_tensor("global_128_frame", global_window)
    ...
    both_buffer.stop()
    torch.cuda.synchronize()
    both_snapshot = cuda_memory_snapshot("global_plus_local_buffer_128", device)
    ...
    describe_tensor("local_128_frame", local_window)
    describe_tensor("global_128_frame", global_window)
    local_mib = tensor_mib(local_window)
    print(f"local_plus_global_128_frame_memory_mib={local_mib + global_mib:.6f}")
    ...
    assert abs(global_mib - 0.5625) < 1e-6
