...
from lv_flow_matching.datasets import ZarrDataset, build_dataloader
from lv_flow_matching.models.fm import FlowMatchingPolicy, build_flow_policy
from lv_flow_matching.provenance import source_provenance
from lv_flow_matching.tools.normalizer import DatasetNormalizer
from lv_flow_matching.trainers.eval_open_loop import evaluate_open_loop
from lv_flow_matching.utils.distributed import DistInfo, barrier, cleanup, init_distributed, is_main_process, reduce_mean
from lv_flow_matching.utils.tensor import move_to_device
from lv_flow_matching.utils.train_utils import cfg_get, detach_scalar_dict, log_hparams_to_tensorboard, set_seed, sync_fm_action_horizon_from_data

ResumeMode = Literal["full", "weights-only"]


def normalize_resume_mode(value: Any) -> ResumeMode:
    mode = str(value or "full").strip().lower()
    if mode not in {"full", "weights-only"}:
        raise ValueError(
            f"invalid train.resume_mode={value!r}; expected 'full' or 'weights-only'"
        )
    return mode  # type: ignore[return-value]


def configured_scheduler_name(train_cfg: Mapping[str, Any]) -> str:
    optimizer_cfg = train_cfg.get("optimizer") or {}
    if not isinstance(optimizer_cfg, Mapping):
        raise TypeError("train.optimizer must be a mapping")
    scheduler_name = str(optimizer_cfg.get("scheduler", "none")).strip().lower()
    if scheduler_name not in {"", "none", "cosine"}:
        raise ValueError(f"unsupported optimizer scheduler: {scheduler_name!r}")
    return scheduler_name


def preflight_resume_checkpoint(cfg: Mapping[str, Any]) -> tuple[dict[str, Any] | None, ResumeMode]:
    """Load and validate resume state before devices, data, or output directories."""
    train_cfg = cfg.get("train")
    if not isinstance(train_cfg, Mapping):
        raise TypeError("train config must be a mapping")
    if "resume_optimizer" in train_cfg:
        raise ValueError(
            "train.resume_optimizer is no longer supported; set "
            "train.resume_mode to 'full' or explicitly to 'weights-only'"
        )

    resume_mode = normalize_resume_mode(train_cfg.get("resume_mode"))
    scheduler_name = configured_scheduler_name(train_cfg)
    resume_path = train_cfg.get("resume_path")
    if not resume_path:
        return None, resume_mode

    state = torch.load(resume_path, map_location="cpu", weights_only=False)
    if not isinstance(state, Mapping):
        raise TypeError(f"resume checkpoint must contain a mapping: {resume_path}")
    ...
    if resume_mode == "full":
        ...
        if bool(train_cfg.get("use_amp", False)):
            required.add("scaler_state_dict")
    missing = sorted(key for key in required if key not in state)
    if missing:
        raise ValueError(
            f"resume_mode={resume_mode} checkpoint is missing required state: {missing}"
        )

    mapping_keys = required - {"epoch", "global_step"}
    invalid = sorted(key for key in mapping_keys if not isinstance(state.get(key), Mapping))
    if invalid:
        raise TypeError(f"resume checkpoint state must be mappings: {invalid}")

    scalar_values = {
        "checkpoint epoch": state["epoch"],
        "checkpoint global_step": state["global_step"],
        "train.epochs": train_cfg.get("epochs", 100),
    }
    ...
    if invalid_scalars:
        raise TypeError(f"resume counters must be integers: {invalid_scalars}")

    checkpoint_epoch = int(scalar_values["checkpoint epoch"])
    global_step = int(scalar_values["checkpoint global_step"])
    epochs = int(scalar_values["train.epochs"])
    if checkpoint_epoch < 0:
        raise ValueError(f"checkpoint epoch must be non-negative, got {checkpoint_epoch}")
    if global_step < 0:
        raise ValueError(f"checkpoint global_step must be non-negative, got {global_step}")
    if epochs < 1:
        raise ValueError(f"train.epochs must be positive, got {epochs}")
    if checkpoint_epoch >= epochs:
        raise ValueError(
            f"checkpoint epoch {checkpoint_epoch} has no remaining work for "
            f"train.epochs={epochs}"
        )

    return dict(state), resume_mode
...
def distributed_all_finite(
    local_finite: bool | torch.Tensor,
    device: torch.device,
) -> bool:
    """Return a rank-consistent finite flag when a process group is active."""
    if torch.is_tensor(local_finite):
        flag = local_finite.detach().to(device=device, dtype=torch.int32).reshape(()).clone()
    else:
        flag = torch.tensor(int(bool(local_finite)), device=device, dtype=torch.int32)
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        torch.distributed.all_reduce(flag, op=torch.distributed.ReduceOp.MIN)
    return bool(flag.item())


def local_gradients_finite_flag(
    parameters: Iterable[torch.nn.Parameter],
    device: torch.device,
) -> torch.Tensor:
    flag = torch.ones((), device=device, dtype=torch.int32)
    for parameter in parameters:
        if parameter.grad is not None:
            flag.mul_(torch.isfinite(parameter.grad).all().to(device=device, dtype=torch.int32))
    return flag
...
def build_dataset_and_loader(
    ...
    if bool(data_cfg.get("use_camera_latent", False)):
        from lv_flow_matching.models.fm.encoders.dino_v2 import resolve_dino_model_name
...
def train_one_epoch(
    ...
    *,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        bad_metrics = {
            key: value for key, value in scalar_metrics.items() if not math.isfinite(value)
        }
        local_output_finite = math.isfinite(loss_value) and not bad_metrics
        if not distributed_all_finite(local_output_finite, device):
            if not math.isfinite(loss_value):
                detail = f"loss={loss_value}"
            elif bad_metrics:
                detail = f"metrics={bad_metrics}"
            else:
                detail = "another distributed rank reported a non-finite output"
            raise FloatingPointError(
                "non-finite training output before backward: "
                f"batch_idx={batch_idx}, global_step={global_step}, {detail}"
            )

        if scaler is not None:
            scaler.scale(loss).backward()
            scaler.unscale_(optimizer)
            local_gradient_flag = local_gradients_finite_flag(policy.parameters(), device)
            if not distributed_all_finite(local_gradient_flag, device):
                local_detail = (
                    "local gradients are non-finite"
                    if not bool(local_gradient_flag.item())
                    else "another distributed rank reported non-finite gradients"
                )
                raise FloatingPointError(
                    "non-finite training gradients after backward: "
                    f"batch_idx={batch_idx}, global_step={global_step}, {local_detail}"
                )
            if grad_clip is not None and grad_clip > 0:
                torch.nn.utils.clip_grad_norm_(
                    policy.parameters(), grad_clip, error_if_nonfinite=True
                )
        ...
        metric_sum["loss"] += loss_value * batch_size
        ...
        postfix = {"loss": f"{loss_value:.4f}", "lr": f"{step_lr:.2e}"}
...
def get_checkpoint_state(
    ...
    *,
    ...
    state = {
        ...
        "config": cfg,
        "flow_matching_source": source_provenance(),
    }
...
def build_scheduler(
    ...
    *,
    ...
    scheduler_name = configured_scheduler_name({"optimizer": optimizer_cfg})
    if scheduler_name in {"", "none"}:
        return None
    if scheduler_name == "cosine":
        return torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=max(1, int(epochs) - int(start_epoch) + 1),
            eta_min=float(optimizer_cfg.get("min_lr", 0.0)),
        )
    raise AssertionError(f"unreachable scheduler name: {scheduler_name!r}")
...
def load_checkpoint(
    ...
    return restore_checkpoint_state(state, policy, optimizer, dataset)
...
def main(cfg: dict) -> None:
    # This must remain the first operation: fail before process groups, devices,
    # datasets, or output directories are created.
    resume_state, resume_mode = preflight_resume_checkpoint(cfg)
    ...
    if resume_state is not None:
        restore_checkpoint_state(
            resume_state,
            policy,
            optimizer if resume_mode == "full" else None,
            dataset,
        )
        ...
        if main_proc:
            print(
                f"Resumed from {resume_path} with mode={resume_mode} at "
                f"epoch={start_epoch}, global_step={global_step}"
            )
            if resume_mode == "weights-only":
                print(
                    "Weights-only resume: policy and normalizer restored; optimizer, "
                    "scheduler, and AMP scaler start fresh at "
    ...
    if main_proc:
        ...
        with open(run_dir / "source_manifest.yaml", "w", encoding="utf-8") as handle:
            yaml.safe_dump(source_provenance(), handle, sort_keys=False)
    ...
    if (
        use_amp
        and resume_mode == "full"
        and resume_state is not None
        and "scaler_state_dict" in resume_state
    ):
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        # Log the rate used for this epoch, then checkpoint the stepped
        # scheduler state so a resume starts with the next epoch's rate.
